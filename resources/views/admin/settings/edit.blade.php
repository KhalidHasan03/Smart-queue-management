<x-app-layout>
    ...
    <form method="POST" action="{{ route('admin.settings.update') }}" class="qc-card p-6 max-w-xl space-y-4">@csrf @method('PUT')
        <div><label class="qc-label">Clinic name *</label><input name="clinic_name" value="{{ old('clinic_name',$settings['clinic_name'] ?? 'QueueCare Clinic') }}" required class="qc-input mt-1"></div>
